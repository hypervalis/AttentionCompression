...
This file is the **detailed chronological work log** (measurements, tables, remote paths, and superseded hypotheses). For a short overview of the idea and how the project evolved, read the repo root **`README.md`**.
...
### Fused Q/K forward (`attention_compression.qk_surgery`)
...
## Low-rank SwiGLU MLP (layer 8, May 2026)

Goal: replace OLMo-1B layer **8** `mlp` (`gate_proj`, `up_proj`, `down_proj`) with rank-capped low-rank branches (same recipe as Q/K: supervised **output PCA** init, then distill). Captures: **`scripts/27_capture_layer_internals.py`** → `ffn_input` only (no per-map activations saved).

**Code:** `src/attention_compression/mlp_lowrank.py` (`LowRankSwiGLU`, `StagedHybridSwiGLU`); **`scripts/48_train_lowrank_mlp.py`** (joint all three maps); **`scripts/51_train_lowrank_mlp_staged.py`** (gate → up → down, one map per stage).

**PCA init (per map):**

| Map | Statistics on |
| --- | --- |
| `gate_proj` | Teacher **`gate_proj(x)`** on `ffn_input` (pre-SiLU) |
| `up_proj` | Teacher **`up_proj(x)`** on `ffn_input` (pre-SiLU) |
| `down_proj` | Teacher **`down_proj(h)`** with `h = act(gate(x)) * up(x)`; when staging, `h` from **hybrid** gate/up if already compressed |

Ranks: **95%** output-variance threshold, **cap 512** per map (all three hit the cap on layer 8).

### Joint training (`48`, `160` windows/bin)

Train/eval: **144 / 16** per bin (960 windows). Loss sweep (`50`): best on PPL was **`both(w=0.25)`** — `loss_kind=both`, `loss_relative_weight=0.25`.

| Config | Capture `ffn_cosine` | PPL ratio (6-window LM smoke, `49`) |
| --- | ---: | ---: |
| Joint low-rank, `both_w0.25` | ~**0.693** | ~**1.086×** |

Artifacts: `/mnt/sdb1/dolma-v1_6-sample/layer08_lowrank_mlp_loss_sweep/both_w0p25/`.

### Staged training — full-MLP loss (`51`, `loss-target mlp`)

Compress and train **one map at a time**; earlier stages frozen. Same **160pb** split as joint.

| Stage (end) | Full-FFN `ffn_cosine` |
| --- | ---: |
| gate only | ~**0.910** |
| + up | ~0.794 |
| + down (final) | ~**0.689** |

PPL ~**1.087×** — same ballpark as joint, but **no gain** on composed FFN cosine; error compounds when up/down are added on a frozen low-rank gate.

Artifacts: `/mnt/sdb1/dolma-v1_6-sample/layer08_lowrank_mlp_staged/`.

### Staged training — isolated per-map I/O (`51`, `loss-target isolated`)

Train each stage on the **exact linear map I/O** (not full SwiGLU output): gate/up on `ffn_input → linear out`; down on **`hybrid.mlp_hidden(x) → down_proj(h)`** vs teacher `down_proj(h)` on the same `h`. Checkpoint selection uses **`stage_linear_cosine`** per stage.

**Data:** **320** windows/bin capture → **312 train / 8 eval** per bin (**1872 / 48** windows total). Remote: `layer08_internals_320pb`.

| Stage | `stage_linear_cosine` | Full-FFN `ffn_cosine` (end of stage) |
| --- | ---: | ---: |
| gate | ~**0.909** | ~0.894 |
| up | ~**0.854** | ~0.739 |
| down | ~**0.853** | ~0.621 |
...
PPL ratio (`49`): ~**1.111×** (slightly worse than joint on 160pb).

Artifacts:

```text
/mnt/sdb1/dolma-v1_6-sample/layer08_internals_320pb/
/mnt/sdb1/dolma-v1_6-sample/layer08_lowrank_staged_isolated_320pb/
  staged_mlp_report.json
  lowrank_mlp_benchmark.json
  pipeline.log
```

**Read:** Isolated training **does** fit each linear map well (~0.85–0.91 cosine on that map’s tensor), but **composition fails**: full-FFN cosine (~0.62–0.69) stays far below per-map linear scores. More data (160→320/bin) did not fix composition; PPL moved slightly the wrong way vs joint. Next levers: short **joint finetune** after staged init (`--finetune-epochs` on `51`), capture **`mlp_hidden`** for down PCA/train, or joint training with better loss — not more staged MLP-only steps alone.

Related negative results (same layer / captures, not low-rank SwiGLU): block factorized FFN (~0.55 capture cosine, ~1.14× PPL); operator-SVD block basis similar.
...
| `src/attention_compression/qk_surgery.py` | Shared patching, fused low-rank Q/K forward, optional **dense Q/K materialization** |
...
| `scripts/48_train_lowrank_mlp.py` | Joint low-rank SwiGLU (`gate`/`up`/`down`) with supervised output-PCA init |
| `scripts/49_lowrank_mlp_benchmark.py` | Single-layer PPL smoke after `apply_lowrank_mlp` |
| `scripts/50_sweep_lowrank_mlp_loss.py` | Loss grid for **48** + benchmark per run |
| `scripts/51_train_lowrank_mlp_staged.py` | Staged compression; `--loss-target isolated` (per-map I/O) or `mlp`; `--train-windows-per-bin 0` = max captured |
...
## CLI / live LM apply (layer 0, May 2026)

**Eval:** `allenai/OLMo-1B-0724-hf`, `selected_train_windows.csv`, **1 window per rarity bin** (6×1024 tokens), same style as script **18** Q/K surgery checks. Checkpoints under `/mnt/sdb1/dolma-v1_6-sample/` (`qk_dense_v/…`, `layer00_bottleneck_ffn_lowrank768_cotrain_5ep/bottleneck_ffn.pt`). Remote JSON: `cli_layer00_lm_apply_benchmark.json`.

| Config | Mean PPL | vs baseline |
| --- | ---: | ---: |
| Baseline | 16.38 | — |
| Q/K only (16 heads, layer 0) | 16.53 | **×1.009** |
| FFN bridge only (`apply_ffn_block`) | 21.25 | **×1.297** |
| Full layer (Q/K + FFN bridge) | 26.07 | **×1.592** |

**Q/K:** CLI `apply` matches prior script **18** layer-0 rerun (~+0.9% PPL). Activation metrics on captures were already strong; live LM drift is small.

**FFN:** Script **32** trains the student on **`R_mimic = ffn_input − mimic(heads)`**, not on raw `ffn_input`. Naive deploy (`student(ffn_input)` or swapping `o_proj` + `mlp` without the bridge) blew up PPL (~10³–10⁴×). The current **`_MLPBridge`** keeps the **teacher `OlmoMLP`** frozen and outputs:

`teacher(ffn_in) + (student(R_mimic) − teacher(R_mimic)) × ‖teacher(ffn_in)‖ / ‖teacher(R_mimic)‖`

because **`teacher_mlp(ffn_in)`** and **`teacher_mlp(R_mimic)`** differ strongly in norm and direction on live tensors (nonlinear MLP). A pre-`o_proj` hook supplies head context to the mimic; **`o_proj` weights are not replaced** in this apply path.

**Gap vs activation eval:** Layer-0 co-train report on captures had **eval `relative_mse` ~0.003**, **cosine ~0.993** on teacher FFN outputs. Live LM at **×1.30** FFN-only shows the bridge is usable for smoke but **not** deployment-ready; stacking Q/K + FFN is worse than the sum of parts would suggest (~1.59× combined vs ~1.01× + ~1.30× naively).

**Limits:** `save_pretrained` after apply may fail on shared custom modules; in-memory apply is the reliable eval path. Fully compressed deploy would need a student that matches **`teacher_mlp(ffn_in)`** end-to-end (or a distilled block without retaining teacher `mlp`).

### Swapped low-rank `o_proj` (script 31 weights, May 2026)

Apply now replaces **`attn.o_proj`** with the **script-31** low-rank mimic (`layerNN_oproj_mimic_ae_lowrank768_160pb/compressed_oproj.pt`), **not** the script-32 co-trained `mimic_oproj` inside `bottleneck_ffn.pt` (that co-trained map has ~**0.03** cosine vs teacher `o_proj` on live head inputs; script-31 has ~**0.96**).

Same 6-window eval as above; remote JSON: `layer00_oproj_swap_benchmark.json`.

| Config | Mean PPL | vs baseline |
| --- | ---: | ---: |
| `oproj` swap only (rank 768) | 17.54 | **×1.071** |
| FFN bridge + swapped script-31 `o_proj` | 28.81 | **×1.759** |
| FFN bridge + teacher `o_proj` (hook mimic) | 21.25 | **×1.297** |

**Read:** rank-768 **`o_proj` swap alone is viable** (~+7% PPL) with real parameter savings on that layer. Combining it with the script-32 MLP bridge hurts more than the hook-only bridge today (co-trained `R_mimic` mimic ≠ script-31 swap).

CLI: `attention-compress apply --target oproj --layer 0 …` or default swapped `o_proj` on `--target ffn` / `layer`.
