#!/usr/bin/env python3
"""PPL smoke: baseline vs low-rank SwiGLU MLP on one layer."""
...
from attention_compression.mlp_lowrank import apply_lowrank_mlp
...
def main() -> None:
    ...
    out: dict = {"model": model_name, "layer": args.layer, "artifact_dir": str(args.artifact_dir)}
    ...
    meta = apply_lowrank_mlp(m, layer_index=args.layer, artifact_dir=args.artifact_dir, device=device, dtype=dtype)
    out["apply"] = meta
    p = perplexity(m, input_ids, device)
    out["lowrank_mlp"] = {**p, "perplexity_ratio_vs_baseline": p["perplexity"] / base}

    out_path = args.output_json or (args.checkpoints / f"layer{args.layer:02d}_lowrank_mlp_benchmark.json")
